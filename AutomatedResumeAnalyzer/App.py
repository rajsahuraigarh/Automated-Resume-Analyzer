...
#             return title
#         else:
#             st.error("Error fetching video: Video object is None")
#             return None
...
def fetch_yt_video(link):
    try:
        ...
        st.error("Watch the video To improve the Resume")
        return None
...
connection = pymysql.connect(host='localhost',user='root',password='RAJ@12raj',db='cv')
...
def run():
    ...
    st.image(img)
    st.title("Automated Resume Analyser")
    st.sidebar.markdown("# Choose User")
    activities = ["User", "Admin"]
    choice = st.sidebar.selectbox("Choose among the given options:", activities)
    link = '[©Developed by Raj ,Abhishek, Gautam & Payal ](https://rajsahuportfolio.web.app/)'
    ...
    if choice == 'User':
            ...
            if resume_data:
                ...
                if res_vid_title is not None:
                  st.subheader("✅ **"+res_vid_title+"**")
                    ...
                    # st.error("Error fetching video title. Please check the YouTube link.")
                    st.video(resume_vid)
        ...
        if st.button('Login'):
            if ad_user == 'Raj' and ad_password == 'Raj123':
                st.success("Welcome ER RAJ SAHU!")
              # Display Data
                ...
                try:
                    ...
                    st.error(f"Error generating pie chart for predicted field recommendation: {e}")
                ...
                try:
                    ...
                    st.error(f"Error generating pie chart for user's experienced level: {e}")
